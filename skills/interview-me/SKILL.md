---
name: interview-me
description: Use when you catch yourself filling an underspecified ask with silent assumptions before any plan, spec, or code exists — "build me X" with no stated who, why now, success, or binding constraint — or when explicitly invoked with "interview me", "grill me", "are we sure?", or "stress-test my thinking". For broad feature design, brainstorming leads; reach for this when the goal is pinning down the user's true intent first.
...
This skill closes the gap before it costs anything. The neighbouring skills assume you already know roughly what you want: `brainstorming` explores requirements and design once the goal is known, `refining-idea` generates variations from an idea, `grilling-with-docs` stress-tests a plan after you've drafted one. Interview-me is the part before all of those, where you ask one question at a time, with your best guess attached, until you can predict what the user is going to say before they say it.
...
- The goal is already known and you need full requirements + design exploration, not just intent — that's `brainstorming`'s job (this skill is the narrower intent-extraction pass; brainstorming may call on it first)
- You already have ≥95% confidence; re-read the stop condition below before assuming you don't

## Loading Constraints

This skill needs a live, responsive user. **Do not invoke in non-interactive contexts** like CI pipelines, scheduled runs, `/loop`, or background subagent runs. If you're in one of those and the ask is underspecified, flag that as a blocker for the user instead of guessing.
...
If the user wants the intent to persist (a multi-session project, a handoff to another collaborator), offer to save it to `docs/superpowers/intent/[topic].md` (create the directory if it doesn't exist). Only save if they confirm.
...
- **`brainstorming`**: the broad pre-build skill, and the one wired to lead on "let's build X". Interview-me is the narrower intent-extraction pass — reach for it (or let brainstorming reach for it) when the goal itself is still fuzzy and you're about to guess. Once intent is confirmed, brainstorming explores requirements and design against it.
- **`refining-idea`**: downstream. If the confirmed intent is "I want X but I don't know how to scope it," hand off to `refining-idea` to generate variations against the now-explicit intent.
- **`writing-plans`**: downstream. If the confirmed intent is concrete ("I want X for Y users with Z success criteria"), hand off to `writing-plans` to write the requirements down and break the work into steps.
- **`grilling-with-docs`**: opposite end of the timeline. Interview-me is pre-decision intent extraction; grilling-with-docs stress-tests a plan *after* you've drafted one. Both catch divergence, but at different moments.
...
- [ ] Any handoff to a downstream skill (`refining-idea`, `writing-plans`) was framed in terms of the confirmed intent, not the original underspecified ask
